...
const Header = () => {
  ...
  // Effect để sync với FavoriteContext nếu cần
  useEffect(() => {
    if (isAuthenticated && user) {
      // Sync session với FavoriteContext nếu cần
      const currentSession = {
        currentUser: user,
        token: localStorage.getItem("session") ? JSON.parse(localStorage.getItem("session")).token : null
      };
      setSession(currentSession);
    }
  }, [user, isAuthenticated, setSession]);

  // Effect để handle cart update events
  useEffect(() => {
    const handleCartUpdate = () => {
      // Handle cart update logic if needed

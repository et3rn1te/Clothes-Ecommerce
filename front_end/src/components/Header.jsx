...
import { FiSearch, FiShoppingCart, FiHeart, FiMenu, FiX, FiMic, FiUser } from "react-icons/fi";
          ...
          <div className="container mx-auto px-6">
            <div className="flex justify-between items-center">
              <div className="flex items-center space-x-8 text-sm">
                <div className="flex items-center text-gray-300 hover:text-white transition-all duration-300 group">
                ...
                <div className="flex items-center text-gray-300 hover:text-white transition-all duration-300 group">
                  <MdLocationOn className="mr-2 text-amber-400 group-hover:scale-110 transition-transform" />
                  <span className="font-light">Miễn phí vận chuyển từ 500k</span>
                </div>
              </div>
              ...
              <div className="flex items-center space-x-6">
                <div className="flex space-x-4">
                  {[FaFacebookF, FaTwitter, FaInstagram, FaLinkedinIn].map((Icon, index) => (
                      <div key={index} className="relative group">
                        <Icon className="text-gray-400 hover:text-white transition-all duration-300 cursor-pointer w-4 h-4 hover:scale-125" />
                ...
                {isLogin ? (
                    <div className="relative">
                      ...
                      {isOpen && (
                          <div className="absolute right-0 mt-3 w-56 rounded-xl shadow-2xl bg-white ring-1 ring-black/5 z-50 border border-gray-100 backdrop-blur-sm">
                            <div className="py-2" role="menu">
                              ...
                              {isAdmin && (
                                  <>
                                    ...
                                    <span className="block px-4 py-2 text-xs text-gray-500 font-semibold uppercase tracking-wider">Quản lý</span>
                    ...
                    >
                      ...
                      <span className="font-light">Đăng nhập</span>
                    </button>
                )}
              </div>
            </div>
        ...
        <nav className="bg-white/95 py-6">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center">
                <Link to="/" className="group">
                  ...
                  <div className="h-0.5 bg-gradient-to-r from-amber-400 to-orange-500 scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left"></div>
                </Link>
              </div>
              ...
              <div className="hidden lg:flex items-center space-x-12">
                {menuItems.map((item, index) => (
                    <div key={index} className="relative group">
                      ...
                      >
                        {item.name}
                        <div className="absolute bottom-0 left-4 right-4 h-0.5 bg-gradient-to-r from-amber-400 to-orange-500 scale-x-0 group-hover:scale-x-100 transition-transform duration-300"></div>
                      </Link>
                    </div>
                ))}
              </div>
              ...
              <div className="flex items-center space-x-6">
                {/* Enhanced Search */}
                <div className={`relative group transition-all duration-300 ${isSearchFocused ? 'transform scale-105' : ''}`}>
                  <div className="relative">
                    <input
                        type="text"
                        value={searchQuery}
                    ...
                    />
                    <FiSearch className="absolute left-4 top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-400 group-focus-within:text-amber-500 transition-colors duration-300" />
                    <button
                        onClick={() => {
                          if (searchQuery.trim()) {
                            navigate(`/search?keyword=${encodeURIComponent(searchQuery)}`);
                ...
                <div className="flex items-center space-x-4">
                  <div className="relative group cursor-pointer" onClick={wishlistClick}>
                    <div className="p-3 rounded-full bg-gray-50 hover:bg-amber-50 transition-all duration-300 hover:shadow-md">
                      <FiHeart className="text-xl text-gray-600 group-hover:text-amber-600 transition-colors duration-300" />
                    </div>
                    {wishlistCount > 0 && (
                        <div className="absolute -top-1 -right-1 bg-gradient-to-r from-red-500 to-pink-500 text-white text-xs rounded-full w-6 h-6 flex items-center justify-center font-semibold transform group-hover:scale-110 transition-transform duration-300 shadow-lg">
                          {wishlistCount}
                        </div>
                    )}
                  </div>
                  ...
                  <div className="relative group cursor-pointer" onClick={cartClick}>
                    <div className="p-3 rounded-full bg-gray-50 hover:bg-amber-50 transition-all duration-300 hover:shadow-md">
                      <FiShoppingCart className="text-xl text-gray-600 group-hover:text-amber-600 transition-colors duration-300" />
                    </div>
                    {cartCount > 0 && (
                        <div className="absolute -top-1 -right-1 bg-gradient-to-r from-amber-500 to-orange-500 text-white text-xs rounded-full w-6 h-6 flex items-center justify-center font-semibold transform group-hover:scale-110 transition-transform duration-300 shadow-lg">
                          {cartCount}
                        </div>
                    )}
                  </div>
          ...
          {isMenuOpen && (
              <div className="lg:hidden fixed inset-0 z-50">
                ...
                <div className="absolute top-0 right-0 w-80 h-full bg-white shadow-2xl transform transition-transform duration-300 overflow-y-auto">
                  <div className="p-6">
                      ...
                      {isAdmin && (
                          <>
                            <div className="border-t border-gray-200 pt-6 mt-6">
                              <span className="block text-gray-500 text-sm font-semibold uppercase mb-4 tracking-wider">Quản lý</span>
                              {adminLinks.map((item, index) => (
                                  <Link
